...
int serialize_device_detail_request(uint32_t token, uint8_t device_id, uint8_t *out_buffer)
{
    out_buffer[0] = MSG_TYPE_DEVICE_DETAIL_CLIENT;  // type = 32
    out_buffer[1] = 5;                               // length = token(4) + device_id(1)
    uint32_t net_token = htonl(token);
    memcpy(out_buffer + 2, &net_token, 4);
    out_buffer[6] = device_id;

    return 7; // 2 header + 5 payload
}
...
int deserialize_packet(const uint8_t *in_buffer, int buffer_len, ParsedPacket *out_packet)
{
    ...
    case MSG_TYPE_SCAN_SERVER:
    {
        ...
        if (out_packet->data.scan_res.num_devices > MAX_DEVICES)
            return -1;
        if (payload_len != 1 + out_packet->data.scan_res.num_devices)
            return -1;
    ...
    case MSG_TYPE_INFO_SERVER:
    {
        ...
        if (out_packet->data.info_res.num_gardens > MAX_GARDENS)
            return -1;
        ...
        for (int i = 0; i < out_packet->data.info_res.num_gardens; i++)
        {
            ...
            if (garden->num_devices > MAX_DEVICES_PER_GARDEN)
                return -1;
            if (offset + garden->num_devices > payload_len)
                return -1;
    ...
    //------------------------------
    // 33 - Device Detail Response
    //------------------------------
    case MSG_TYPE_DEVICE_DETAIL_SERVER:
    {
        DeviceDetailResponse *detail = &out_packet->data.device_detail_res;
        int offset = 0;
        
        // Minimum size check (device_id + sensor data + config + thresholds + counts + direct control)
        if (payload_len < 20) return -1;
        
        // Device ID
        detail->device_id = payload[offset++];
        
        // Sensor Data (4 bytes)
        detail->soil_moisture = payload[offset++];
        detail->npk_n = payload[offset++];
        detail->npk_p = payload[offset++];
        detail->npk_k = payload[offset++];
        
        // Config Parameters (6 bytes)
        detail->fert_concentration = payload[offset++];
        detail->fert_volume = payload[offset++];
        detail->power_lamp = payload[offset++];
        uint16_t interval_net;
        memcpy(&interval_net, &payload[offset], 2);
        detail->interval_time = ntohs(interval_net);
        offset += 2;
        
        // Thresholds (5 bytes)
        detail->humidity_min = payload[offset++];
        detail->humidity_max = payload[offset++];
        detail->npk_n_min = payload[offset++];
        detail->npk_p_min = payload[offset++];
        detail->npk_k_min = payload[offset++];
        
        // Water schedules
        detail->num_water_times = payload[offset++];
        if (detail->num_water_times > MAX_SCHEDULE_SLOTS) return -1;
        for (int i = 0; i < detail->num_water_times; i++) {
            if (offset + 4 > payload_len) return -1;
            uint32_t time_net;
            memcpy(&time_net, &payload[offset], 4);
            detail->water_times[i] = ntohl(time_net);
            offset += 4;
        }
        
        // Light schedules
        detail->num_light_schedules = payload[offset++];
        if (detail->num_light_schedules > MAX_SCHEDULE_SLOTS) return -1;
        for (int i = 0; i < detail->num_light_schedules; i++) {
            if (offset + 8 > payload_len) return -1;
            uint32_t on_net, off_net;
            memcpy(&on_net, &payload[offset], 4);
            detail->light_on_times[i] = ntohl(on_net);
            offset += 4;
            memcpy(&off_net, &payload[offset], 4);
            detail->light_off_times[i] = ntohl(off_net);
            offset += 4;
        }
        
        // Direct Control States (3 bytes)
        if (offset + 3 > payload_len) return -1;
        detail->direct_pump = payload[offset++];
        detail->direct_light = payload[offset++];
        detail->direct_fert = payload[offset++];
    ...
    {
        if (payload_len < 5)
            return -1; // 4 bytes token + 1 byte garden_id
    ...
    {
        if (payload_len < 6)
            return -1; // 4 bytes token + 1 byte garden_id + 1 byte dev_id
    ...
    {
        if (payload_len < 5 + APPID_FIXED_LENGTH)
            return -1;
        ...
        if (payload_len < 5 + APPID_FIXED_LENGTH + old_pass_len)
            return -1;
