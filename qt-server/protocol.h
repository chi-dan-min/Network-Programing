...
#define MSG_TYPE_DEVICE_DETAIL_CLIENT       32          // 0x20
#define MSG_TYPE_DEVICE_DETAIL_SERVER       33          // 0x21
...
#define MAX_SCHEDULE_SLOTS 10
#define SENSOR_NA_VALUE 0xFF  // Value indicating N/A for sensor readings

typedef struct {
    uint8_t device_id;
    
    // Sensor Data (0xFF = N/A)
    uint8_t soil_moisture;      // 0-100 or 0xFF
    uint8_t npk_n;              // 0-255 or 0xFF
    uint8_t npk_p;              // 0-255 or 0xFF
    uint8_t npk_k;              // 0-255 or 0xFF
    
    // Config Parameters
    uint8_t fert_concentration; // g/L (0-255)
    uint8_t fert_volume;        // L (0-255)
    uint8_t power_lamp;         // % (0-100)
    uint16_t interval_time;     // minutes
    ...
    // Auto Schedules
    uint8_t num_water_times;
    uint32_t water_times[MAX_SCHEDULE_SLOTS];
    uint8_t num_light_schedules;
    uint32_t light_on_times[MAX_SCHEDULE_SLOTS];
    uint32_t light_off_times[MAX_SCHEDULE_SLOTS];
    
    // Direct Control States
    uint8_t direct_pump;        // 0=OFF, 1=ON
    uint8_t direct_light;
    uint8_t direct_fert;
...
typedef struct {
    ...
    union {
        ...
        DeviceDetailRequest device_detail_req;
        DeviceDetailResponse device_detail_res;
    } data;
} ParsedPacket;
...
/**
 * @brief Gói tin Device Detail Request (Client -> Server)
