...
// Global callback for packet display
std::function<void(const char*, const uint8_t*, int)> packet_display_callback = nullptr;

// Helper function to format packet as hex string
std::string format_packet_buffer(const uint8_t *buffer, int len)
{
...
void print_buffer(const char *title, const uint8_t *buffer, int len)
{
    ...
    if (packet_display_callback) {
        packet_display_callback(title, buffer, len);
    }
    
    // Still print to cout for debug
...
bool client_login(int sockfd, uint32_t &token, const std::string& appID, const std::string& password)
{
    ...
    // --- Gửi Connect Request ---
    packet_len = serialize_connect_request(appID.c_str(), password.c_str(), send_buffer);
    ...
    {
        // Login failed - let GUI handle the error
        return false;
    }

    default:
    {
        cerr << "Unexpected packet type: " << (int)packet.type << endl;
        return false;
    }
    }
}
